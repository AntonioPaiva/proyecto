...
public class TareaDAOImpl implements TareaDAO{
	...
	public TareaDTO getTareaPorId(int id) {
		...
		String sql = "SELECT id, modulo, proyecto, usuario, " +
				"tipo, fecha_ini, fecha_fin, fecha_culm, avance FROM tarea WHERE id="+id;
		PreparedStatement stm; 
		ResultSet rs = null;
		try {
			stm = ManejadorBD.getCon().prepareStatement(sql);
	...
	public List<TareaDTO> cartaGantt(Integer idProyecto) {
		String sql = "SELECT t.id, t.avance, t.modulo, t.proyecto, t.usuario, t.tipo, t.fecha_ini, t.fecha_fin, t.fecha_culm " +
				"FROM tarea t inner join modulo m on (t.modulo = m.id) " + (idProyecto == null ? "" : " where t.proyecto = " + idProyecto) + " order by m.componente desc";
		PreparedStatement stm;
		ResultSet rs = null;
		List<TareaDTO> listaTarea = new ArrayList<TareaDTO>();
		TareaDTO tarea = null;
